...
const typedUUIDv4 = uuidv4;
const finalEventRegex = /(END|STOP)$/;
const dbEngine = `redux`;
export class AnalyticsTracker {
    constructor({ componentId, gatsbyCliVersion, } = {}) {
        this.store = new EventStorage();
        this.debouncer = {};
        this.metadataCache = {};
        this.defaultTags = {};
    ...
    aggregateStats(data) {
        ...
        const stdDev = Math.sqrt(data.reduce((acc, x) => acc + Math.pow(x - mean, 2), 0) /
            (data.length - 1)) || 0;
        const skewness = data.reduce((acc, x) => acc + Math.pow(x - mean, 3), 0) /
            data.length /
            Math.pow(stdDev, 3);
        return {
            count: data.length,
