...
describe(`Errors Helpers`, () => {
    describe(`sanitizeError`, () => {
        ...
        it(`Sanitizes current path from error stacktraces`, () => {
            ...
            expect(sanitizedErrorString).toEqual(expect.stringContaining(errorMessage));
            expect(sanitizedErrorString).toEqual(expect.not.stringContaining(process.cwd().replace(`\\`, `\\\\`)));
        });
        ...
        `,
            ...
            expect(sanitizedErrorString).toEqual(expect.not.stringContaining(`sidharthachatterjee`));
    ...
    describe(`cleanPaths`, () => {
        it.each([`gatsby-config.js`, `src/pages/index.js`])(`should clean path on unix: %s`, filePath => {
            ...
            expect(cleanPaths(errorMessage, `/`)).toBe(`This path $SNIP/${filePath} is a test $SNIP/${filePath}`);
            mockCwd.mockRestore();
        });
        it.each([`gatsby-config.js`, `src\\pages\\index.js`])(`should clean path on windows: %s`, filePath => {
